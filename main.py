...
class Main():
    def __init__(self, size, title, icon_path, fps):
        self.screen = PygameTools.makescreen(
            size[0], size[1], title, icon_path)
    ...
    def main(self):
        ...
        def gradient(col1, col2, size):
            diff = (max(col1[0], col2[0])-min(col1[0], col2[0]), max(col1[1], col2[1]) -
                    min(col1[1], col2[1]), max(col1[2], col2[2])-min(col1[2], col2[2]))
            mid = (min(col1[0], col2[0])+round(diff[0]/2), min(col1[1], col2[1]
                                                               )+round(diff[1]/2), min(col1[2], col2[2])+round(diff[2]/2))
        ...
        def getdetailed():
            ...
            topdetailimage = gradient(
                topdetailtop, topdetailbottom, (60, 60)).rotate(90).resize((60, 1))
            bottomdetailtop = self.cycle.getpixel((self.phase, 1))
            bottomdetailbottom = self.cycle.getpixel((nextphase, 1))
            bottomdetailimage = gradient(
                bottomdetailtop, bottomdetailbottom, (60, 60)).rotate(90).resize((60, 1))
        ...
        while self.running:
            ...
            gradimagepyg = pygame.image.fromstring(
                gradimage.tobytes(), (800, 800), "RGB")
            PygameTools.image(self.screen, gradimagepyg, 0, 0, 800, 800)
            PygameTools.makesquare(self.screen, 800, 0,
                                   200, 800, 115, 115, 115)
            PygameTools.makesquare(self.screen, 815, 15,
                                   170, 50, 200, 200, 200)
            pausetext = ""
            if paused:
                pausetext = "Unpause"
                ...
                pausetext = "Pause"
            PygameTools.centeredtext(
                self.font, self.screen, pausetext, 815, 15, 170, 50, 255, 255, 255)
            PygameTools.makesquare(self.screen, 815, 80,
                                   170, 50, 200, 200, 200)
            PygameTools.centeredtext(
                self.font, self.screen, "Save", 815, 80, 170, 50, 255, 255, 255)
            PygameTools.makesquare(
                self.screen, 815, 155, (45-(self.cycle.width-45))*2+10, 10, 200, 200, 200)
            PygameTools.makesquare(
                self.screen, 815+(tick*2), 145, 10, 30, 130, 130, 130)
            PygameTools.text(self.font, self.screen,
                             f"{tick}", 950, 145, 255, 255, 255)
            PygameTools.makesquare(self.screen, 815, 200,
                                   self.cycle.width*2+10, 10, 200, 200, 200)
            PygameTools.makesquare(
                self.screen, 815+(self.phase*2), 190, 10, 30, 130, 130, 130)
            PygameTools.text(self.font, self.screen,
                             f"{self.phase}", 950, 190, 255, 255, 255)
            if mouse:
